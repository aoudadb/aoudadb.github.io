...
|------------------|---------------|-----------------|------------------------|------------------|-------|
| _Unreleased_ | `2` | `≥ 0.1.23` (no TS surface this train — the new `GET /api/server/memory` fields are additive) | `≥ 0.1.32` | `≥ 0.0.26` (pin stays `@aouda/client` **0.1.22**) | **P53 — nothing enters hot without asking.** A flush writes exactly **one** representation (hot or cold, decided at flush time; `ColdPreferred` always cold). ⚠️ Three named behaviour changes: `ColdPreferred` rows read from disk after flush; an **edge/vector** bulk load writes cold by default (`Aouda:BulkLoad:Temperature`); rows flushed cold under pressure read from disk. `Aouda:Memory:FlushAlwaysHotFirst=true` restores the previous flush. New aggregate hot ceiling across databases (`Aouda:Memory:ProcessHotCeilingEnabled`), inert where the per-database ceilings already sum correctly. A `HotOnly` pin is now a charged reservation, refused on **its own** budget and validated at declaration. Writer pacing ships **off** (`Aouda:Memory:HotPacingEnabled`). `GET /api/server/memory` gains 11 per-database and 5 process fields (all additive; `ErrorCode` values unchanged). |
