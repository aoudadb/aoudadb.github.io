...
|------------------|---------------|-----------------|------------------------|------------------|-------|
| `0.1.9` | `1` | `≥ 0.1.14` (`totalMatches`; `orderByIndex` / `whenParamPresent` / `orderByChoices` / `count`; `collapse_inserts`) | `≥ 0.1.9` | `≥ 0.0.18` | P40 data-plane completeness: MQ `dataPlaneAccess` + `outputName` vocabulary (`D-31` break); named-query count / optional predicates / bounded sort; conflation that conflates; computed MQ ranking; row-index diagnostics. Studio pin remains `@aouda/client` **0.1.13** until an explicit Studio bump. |
