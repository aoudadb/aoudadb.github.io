...
|------------------|---------------|-----------------|------------------------|------------------|-------|
| `0.1.22` | `1` | `≥ 0.1.21` (MQ wait APIs / `mqRebuildStatus`; pin after npm — published line is still **0.1.20**) | `≥ 0.1.22` | `≥ 0.0.24` (pin `@aouda/client` **0.1.21** after npm) | **BL-432** derived-column predicate cold/hot scan gap; **BL-424** hot coalesce row tear; **BL-427** MQ queue-drop staleness (`isStale` / `staleReason`; health Degraded — C# mirrored, TS types **BL-428**); **BL-419** MQ refresh await + client wait APIs, `MaxConnections` default `0`, `RefreshAwaitTimeout` 120s; **P46** single-pass bulk-load MQ materialization; **BL-423** bounded startup/shutdown; WS auth handshake harden. |
