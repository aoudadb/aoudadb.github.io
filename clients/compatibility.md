...
|------------------|---------------|-----------------|------------------------|------------------|-------|
| `0.1.8` | `1` | `≥ 0.1.12` (`namedQueries.subscribe`; `$upper`…`$cast` call operators) | `≥ 0.1.8` | `≥ 0.0.17` | BL-173 typed subscribe-by-hash; BL-174 `materializedQueries` in `aouda.schema.json`; BL-175 `call` string/rounding functions; BL-171/172 bulk-load Timestamp ISO-8601 and NULL validity. Studio pin stays `@aouda/client` `0.1.11` until a Studio bump (Studio does not call the new subscribe API). |
