...
|------------------|---------------|-----------------|------------------------|------------------|-------|
| `0.1.16` | `1` | `≥ 0.1.17` (`restore({ backupId, targetTime })`; `pitrEligible`) | `≥ 0.1.16` (`RestoreAsync(RestoreBackupRequest)`) | `≥ 0.0.21` (pin `@aouda/client` **0.1.17**) | **BL-186 complete.** PITR on recovery path; recoverable-window metrics/`db inspect`; PITR over HTTP and both SDKs; final docs. **BL-343** derived-column row-window synthesis. Studio backup UI stays exact-restore-only. |
