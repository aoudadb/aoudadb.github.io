...
|------------------|---------------|-----------------|------------------------|------------------|-------|
| *next train (Unreleased)* | `1` | patch changeset pending (`restore({ backupId, targetTime })`) | `RestoreAsync(RestoreBackupRequest)` on current Unreleased | copy-only: backup page no longer claims PITR needs future server support | **BL-186 complete.** PITR over HTTP/SDK; docs rewritten from the S01 honesty pass. Cut this row when the train ships — do not treat this line as a published version. |
