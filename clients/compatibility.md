...
|------------------|---------------|-----------------|------------------------|------------------|-------|
| `0.1.34` | `2` | `≥ 0.1.23` (no TS surface this train) | `≥ 0.1.34` | `≥ 0.0.26` (pin stays `@aouda/client` **0.1.22**) | **BL-609** a bulk-load maintenance path no longer creates unused MQ build tables. **BL-610** shipped binaries default to `Warning` (operator `Logging:LogLevel` still wins). **BL-612** no `appsettings` file pins `MaxTotalRamBytes` — host derivation is the only correct default. |
| `0.1.33` | `2` | `≥ 0.1.23` (no TS surface this train — `GET /api/server/memory` fields and MQ `amplification` are additive) | `≥ 0.1.33` | `≥ 0.0.26` (pin stays `@aouda/client` **0.1.22**) | **P53 — nothing enters hot without asking.** A flush writes exactly **one** representation (hot or cold, decided at flush time; `ColdPreferred` always cold). ⚠️ Three named behaviour changes: `ColdPreferred` rows read from disk after flush; an **edge/vector** bulk load writes cold by default (`Aouda:BulkLoad:Temperature`); rows flushed cold under pressure read from disk. `Aouda:Memory:FlushAlwaysHotFirst=true` restores the previous flush. New aggregate hot ceiling across databases (`Aouda:Memory:ProcessHotCeilingEnabled`), inert where the per-database ceilings already sum correctly. A `HotOnly` pin is now a charged reservation, refused on **its own** budget and validated at declaration. Writer pacing ships **off** (`Aouda:Memory:HotPacingEnabled`). `GET /api/server/memory` gains 11 per-database and 5 process fields (all additive; `ErrorCode` values unchanged). **BL-311 / BL-519 / BL-495** crash reattach for materialized-query results. **BL-523** per-class memory admission on by default. **BL-515** PK-less `filter` MQ keeps duplicates. **BL-586** MQ result residency is `Auto`, not a query-type pin. MQ status gains `amplification`. |
| `0.1.32` | `2` | `≥ 0.1.23` (no TS surface this train) | `≥ 0.1.32` | `≥ 0.0.26` (pin stays `@aouda/client` **0.1.22**) | **BL-565** decimal overflow no longer pins hot segments (plain-codec fallback). **BL-566** forced demotion skips a segment already proven unbuildable. **BL-567** a memory refusal names the untracked gap (`ResidentAttributedBytes`). **BL-568** overlapping bulk-load MQ rebuilds coalesce (one in-flight scan per source table). |
| `0.1.31` | `2` | `≥ 0.1.23` (no TS surface this train — batch-writer parity is **BL-543**) | `≥ 0.1.31` (`CreateBatchWriter`) | `≥ 0.0.26` (pin stays `@aouda/client` **0.1.22**) | **P52** — smaller pieces, not a bigger ceiling: ingest-fed MQ builds partition instead of full-table scan (S01); a memory refusal no longer buys a scan (S02); a hard-limited heap paces ingest instead of killing the process (S03). **BL-560** / **BL-415** keyed ingest no longer collapses on cold segments. **BL-529** ordinary update no longer 500s as Duplicate PK. **BL-530** / **BL-563** chained materialized queries. **BL-545** refuse replication on a table that writes no WAL. **BL-552** elastic-share activity term on by default. |
| `0.1.30` | `2` | `≥ 0.1.23` (`MQ_NOT_READY` retryable; pin after npm — published line still `0.1.22`) | `≥ 0.1.30` | `≥ 0.0.26` (pin `@aouda/client` **0.1.23** after npm) | **P51** — a bulk load costs its rows, not its table (BL-517). **P50 S09** bounded `Filter` build (BL-503). **BL-525** freshness: omitted `onExceeded` is `wait` on Primary/Standalone; `MQ_NOT_READY` (409) when a materialized query is not Ready. **BL-521** unsigned/float/decimal predicates. **BL-509** L2 hot-index retirement. |
| `0.1.29` | `2` | `≥ 0.1.22` (no TS surface this train — `workingSetTruncated` is C#/`GET` MQ status only; TS parity is a follow-up) | `≥ 0.1.29` | `≥ 0.0.26` | **P50** — a materialized query result is an ordinary table (one read/write path; `QueryMaterializedAsync` Timestamp arrives as `DateTime`; `UpdateMode.Sync` means never-dropped, not dispatch-thread). **BL-502** opt-in bounded Top-N-per-group (`TopNPartitioningEnabled` default off; `workingSetTruncated`). **BL-508** tombstoned PK re-insert, MFA verify in-place, false ingest 503s. Standalone token mint no longer lags itself. |
| `0.1.28` | `2` | `≥ 0.1.22` (no TS surface — review follow-ups on 0.1.27 durability paths) | `≥ 0.1.28` | `≥ 0.0.26` | **PR #348 review follow-ups** — deletion-mask write failure fails the flush so deleted rows cannot reappear (BL-507); demoted edge tables keep Kind/EdgeConfig in the segment manifest; column-less demoted segments still get a manifest (BL-439 follow-ups). |
| `0.1.27` | `2` | `≥ 0.1.22` (no TS surface for this train — server durability/MQ fixes and aggregate partitioning settings are config/engine; existing clients keep working) | `≥ 0.1.27` | `≥ 0.0.26` | **Durability and MQ correctness** — backup records WAL-root incarnation for PITR (BL-330/438); demoted cold segments write manifests and rebuild correctly (BL-439/506); checkpoint sync staging path fixed (BL-328); flush fails on hot-segment write error and does not advance redo past in-flight writers (BL-504/500); MQ watermark/token/create-race fixes (BL-487/501/498); over-budget aggregate MQ partitioning (BL-477/491). |
| `0.1.26` | `2` | `≥ 0.1.22` (no TS surface for this train — admin memory/CPU fields and named-mutation `{ "value": }` are additive wire; existing clients keep working) | `≥ 0.1.26` | `≥ 0.0.26` | **P49 ResourceGovernance** — work is classified (`Interactive` / `Streaming` / `Ingest` / `Background` / `Maintenance`); background yields to live traffic and expands when quiet; a host with headroom gets a faster Aouda (`resourceMode` `Constrained`/`Balanced`/`Abundant`). `GET /api/server/memory` gains `reservedByClass`, `ceilingCountedByClass`, `cpu` (read `source`), `workInFlight`, `resourceMode` / `headroomRatio`. `HotOnly` tables declare `residency.hotOnlyBackstop` (`RefuseWrites` default). Raw-HTTP `PinAllInMemory` is now `400`. Queries over `Aouda:Query:MaxResultRows` and named-query batches over the class ceiling refuse with typed `503`. **BL-479** — named-mutation `values`/`set` may spell `{ "value": <literal> }`; other object nodes fail apply with `NAMED_MUTATION_VALUE_NODE_INVALID`. |
