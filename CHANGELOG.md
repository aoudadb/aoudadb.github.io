...
- **A materialized query's result table is no longer pinned in RAM because of its query type (BL-586).** `aggregate`, `latestPerKey` and `firstPerKey` results were created `StorageTemperature = HotOnly` on the reasoning — written in the engine source — that they are *"small, frequent access"* and *"usually small"*. They are not bounded that way: an `aggregate` result holds one row per group and a `latestPerKey` one row per key, so both grow with the source's cardinality. A measured eight-query fan-out held **2 199 815 groups**, and per-minute candles over a few thousand instruments is an ordinary shape. **The default is now `Auto`**, the same default an ordinary table has.
  ...
  - **An explicit `storage.storageTemperature` is honoured exactly as before**, on every version, which is why declaring one is the portable thing to do. `Aouda:Memory:MaterializedResultHotOnlyByQueryType=true` restores the per-type table whole.
  - 🔎 **The symptom this produced was a `503` on an unrelated write.** If your hot tier is fuller than your declared tables account for, read `pinnedHotBytes` on `GET /api/server/memory`. New guidance on which temperature to pick per query shape: [Materialized queries §2.3.1](guides/materialized.md#where-a-materialized-querys-result-lives), [Market data](guides/market-data.md#mq-residency), [Sizing](guides/sizing.md#the-pin-you-did-not-declare-materialized-query-results), [Hot/Cold](guides/hot-cold.md#mq-result-tables).
...
- **Insert `autoIncrement`: omitting the column now means `0` — generate (BL-429).** ✅ **Shipped.** The docs had said since 0.1.21 that `0` and omitted were equivalent; the engine accepted one and rejected the other with `400 INVALID_REQUEST` (`Missing required column '…'`), so a client that simply left the identity column out of its payload got a 400 for following the documentation. Reported by a pilot customer. ⚠️ **On server `0.1.32` and earlier the column must still be present holding `0`.** Two boundaries are unchanged on every version: an explicit `null` is not a request to generate, and under `identityInsert: true` omitting the column remains an error. [HTTP API insert](reference/http-api.md#post-apidatabasesdbtablesnamerows), [Named queries — batch insert](guides/named-queries.md#batch-insert-batchparam).
