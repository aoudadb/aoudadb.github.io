...
- **A named mutation's `values` / `set` entry has exactly three forms, and a typo now fails at schema apply (BL-479).** A literal (any JSON value that is not an object), a `{ "param": "<name>" }` hole, or — new — a `{ "value": <literal> }` constant. The object shape belongs to those two one-key forms and nothing else, because `{ "param": … }` already owns it: a bare object constant is indistinguishable from a mistyped hole. Anything else (`{ "valu": 0 }`, both keys at once, `{}`, an object nested under `value`) fails apply with `NAMED_MUTATION_VALUE_NODE_INVALID`, naming the field, the column and the keys found. Reported by a pilot customer, whose `{ "value": 0 }` on an `autoIncrement` key returned a bare `400 INVALID_REQUEST` — `Cannot convert JsonElement (ValueKind=Object)` — from deep inside column conversion on every call, naming neither the column nor the mutation. A definition already deployed with such a node now fails with `NAMED_MUTATION_BIND_FAILED` naming the column instead. A JSON document destined for a `String` column goes in as a JSON string, not a bare object. [Named queries — writing `values` and `set`](guides/named-queries.md#writing-values-and-set), [HTTP API — named mutations](reference/http-api.md#named-mutations).
