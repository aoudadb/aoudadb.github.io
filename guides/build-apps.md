...
| Getting data back | Incremental backups, exact restore, retention, S3-compatible targets, point-in-time recovery over HTTP and both SDKs (Studio UI is exact-restore-only) | [Backup and restore](backup.md) |
...
| Multi-tenant row isolation (SaaS), tenants each holding ~1 M rows | `partitionKey` + `authMode: auth-db-pls` (or `jwt-claim` PLS) | [Partitioning](partitioning.md) · [Data authorization](../auth/authorization.md) |
| Multi-tenant row isolation, tenants holding far less than that | **No `partitionKey`** — `auth-db-rls` with a named resolver | [Should this table have a partition key?](partitioning.md#should-this-table-have-a-partition-key-p45) |
...
| Users who span many tenants/rooms/tickers, each holding ~1 M rows | `auth-db-pls` with `permissionDimension` and partition grants | [Mode 2: auth-db-pls](../auth/authorization.md#194-mode-2-auth-db-pls-enhanced-pls) |
| Users who span many rooms/tenants that are **individually small** (the common chat/collaboration shape) | **No `partitionKey`** — `auth-db-rls` with a `PartitionGrant` rule. Same grants, minted and revoked the same way, without the partition-filter guard | [The unpartitioned variant](../auth/authorization.md#the-unpartitioned-variant--same-grants-no-partition-key) |
